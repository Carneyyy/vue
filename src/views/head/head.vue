   ...
   <div id="header" class="new_header">
    ...
    <div class="hd_main cle">
      ...
      <div class="head_search_hot">
        <span>热搜榜：</span>
        <router-link :to="'/app/home/search/'+item.keywords" v-for="item in hotSearch" :key="item.id">{{item.keywords}}</router-link>
      </div>
      <div class="intro">
        <ul>
          <li class="no1">
      ...
      <div class="hd_nav_bd cle">
        <div class="main_nav main_nav_hover" id="main_nav">
          <div class="main_nav_link" @mouseover="overAllmenu" @mouseout="outAllmenu">
            ...
            <!-- display控制一级类别菜单显示隐藏 -->
            <div class="main_cata" id="J_mainCata" v-show="isShowAll">
              <ul>
                  ...
                  <div class="J_subCata" v-show="showChildrenMenu===index" style="left: 215px; top: 0px;">
                    <div class="J_subView" >
                      <div v-for="list in item.sub_cat" :key="list.id">
                        <dl>
                          <dt>
                            <router-link :to="'/app/home/list/'+list.id">{{item.name}}</router-link>
                          </dt>
                          <dd>
          ...
          <div class="list" v-show="showShopCar">
            <div class="data">
              <dl v-for="(item,index) in goodsList.goods_list" :key="index">
                ...
                <dd>
                  <h4><router-link :to="'/app/home/productDetail'+item.goods.id" target="_blank">{{item.goods.title}}</router-link></h4>
...
<script>
import {getCatrgory, getHostSearch, deleteShopCart} from '../../api/api'
import {mapGetters} from 'vuex'
export default {
  data () {
    return {
      isShowVip: false, // 会员中心子菜单显示控制
      isShowAll: false, // 全部端口显示控制
      showShopCar: false, // 购物车显示控制
      showChildrenMenu: -1, // 二级三级菜单显示控制
      allMenuLabel: [], // 各级菜单列表
      hotSearch: [],
      searchWord: ''// 搜索词
    }
