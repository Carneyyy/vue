            ...
            <form id="mobile_register_form" autocomplete="off">
              ...
              <div class="clearfix">
                <div class="form-group marb8 verify-code">
                  <label>短信验证码</label>
                  <input id="jsPhoneRegCaptcha" type="text" placeholder="输入手机验证码">
                </div>
                <input class="verify-code-btn sendcode" type="button" id="jsSendCode" :value="getMassageText" @click="seedMessage">
              </div>
...
export default {
  ...
  methods: {
    seedMessage () {
      ...
      function settime () {
        if (countdown === 0) {
          that.getMassageText = '免费获取验证码'
          countdown = 60
          return
        } else {
          that.getMassageText = '重新发送 (' + countdown + ')'
          countdown--
        }
        // 开定时器启动倒计时
        setTimeout(function () {
    ...
    isRegister () {
      register({
        ...
        this.error.mobile = error.mobile ? error.mobile[0] : ''
        this.error.username = error.username ? error.username[0] : ''
