<template>
  <div class="sidebar">
    <div class="cate-menu" id="cate-menu">
      <!-- 一级类别 -->
      <h3 v-if="isobject"><a href=""><strong>{{currentCategoryName}}</strong><i id="total_count">{{proNum}}</i></a></h3>
      <dl>
        <div v-for="item in cateMenu" :key="item.id">
          <!-- 二级类别菜单 -->
          <dt>{{item.name}}</dt>
          <!-- 三级类别 -->
          <dd v-for="sumitem in item.sub_cat" :key="sumitem.id">
            <a @click="changeMenu(sumitem.id)">{{sumitem.name}}</a>
          </dd>
        </div>
      </dl>
    </div>
