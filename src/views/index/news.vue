    ...
    <div class="w-max ct bgwh mb30 ovh border-eee">
      ...
      <div class="newopro-r fr">
        ...
        <ul class="newgoods_fastbuy">
          <li class="prolist-cent clearfix have_num" v-for="item in newporo" :key="item.id">
            ...
            <div class="prolist-r fl">
              <h3 class="ft14 c333 bold">
                <a href="list.html" :title="item.name" target="_blank">{{item.name}}</a>
              </h3>
              <p><em class="c333"></em>{{item.goods_brief}}
              </p>
              <div>
  ...
  methods: {
    getOpro () {
      getGoods({
        ...
        this.newporo = response.data.results
      }).catch(error => {
        console.log(error)
      })
    }
