  ...
  <div id="main">
    <div class="top-next cle">
      <div class="fl">
        ...
        <a href="order.html" class="btn" id="checkout-top">&nbsp;去结算&nbsp;</a>
      </div>
    </div>
    <div class="cart-box" id="cart-box">
      ...
      <div class="goods-list">
        <ul>
          ...
          <li class="cle hover" style="border-bottom-style: none;" v-for="(item,index) in goods_list" :key="index">
            <div class="pic">
              <a target="_blank"> <img :alt="item.goods.title" :src="item.goods.image"></a>
            </div>
            <div class="name">
              <a target="_blank">{{item.goods.title}}</a>
              <p></p>
            </div>
            <div class="price-xj">
      ...
      <div class="fd cle">
        ...
        <div class="fr" id="price-total">
          <p><span id="selectedCount">{{goods_list.length}}</span>件商品，总价：<span class="red"><strong id="totalSkuPrice">￥{{totalPrice}}元</strong></span>
          </p>
        </div>
        <div class="extr">
          <div class="address">
            ...
            <ul>
              ...
              <!-- 收货人地址 -->
              <li @click="selctAddr(item)" v-for="item in addressInfo" :key="item.id" :class="{'addressActive': addressActive===item.id}">
          ...
          <div class="pay">
            ...
            <p class="payWrap">
              <!-- 支付方式 -->
              <img v-for="item in payWrapList" :key="item.id" :src="item.img" :class="{'payWrapActive': payWrapActive===item.id}" @click="selctPay(item.id)">
            </p>
          </div>
        </div>
        <textarea placeholder="请输入留言" style="margin-top: 10px; height:50px;width: 100%;" v-model="post_script"></textarea>
        <p class="sumup">
          <a @click="balanceCount" class="btn">去结算</a>
        </p>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    reduceCartNum (index, id) { // 删除数量
      if (this.goods_list[index].nums <= 1) {
        ...
        updateShopCart(id, {
          nums: this.goods_list[index].num - 1
        }).then(response => {
          // console.log(response.data)
    ...
    SetTotalPrice () {
      ...
      for (var i = 0; i < goodslist.length; i++) {
        totalprice += goodslist[i].nums * goodslist[i].goods.shop_price
      }
      this.totalPrice = totalprice
    },
    delAll () { // 清空购物车
